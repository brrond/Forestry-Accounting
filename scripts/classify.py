...
f1 = list(filter(None, [f if '_SR_stac.json' in str(f) else None for f in Path(FIRST).glob('*')]))
f2 = list(filter(None, [f if '_SR_stac.json' in str(f) else None for f in Path(SECOND).glob('*')]))
